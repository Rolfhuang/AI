...
class CustomPlayer:
    ...
    def move(self, game, time_left):
        ...
        # except TimeoutError:
        #     return best_move
        my_move=game.get_player_moves(self)
        count=0
        if len(my_move)<6:
            current_depth=self.search_depth+1
        else:
            current_depth=self.search_depth
        for i in range(current_depth):
            if time_left()<50: break
            best_move, utility = alphabeta(self,game, time_left, i, alpha=float("-inf"), beta=float("inf"))
            # best_move, utility = minimax(self, game, time_left, depth=i)
            # if utility>best_val:
    ...
    if my_turn:
        for move in legal_moves:
            if move in killer_move:
                forecast_game, is_over, winner = game.forecast_move(move)
                if is_over:
                #     if winner==game.get_inactive_players_queen(player):
                #         break
                    return move, player.utility(game, forecast_game)
                    # break
